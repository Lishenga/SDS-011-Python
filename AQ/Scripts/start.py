    ...
    if V.OPCON == "ON":
        for sen in SENSORS:
            ...
            if NAMES == "":
                ...
                # solution to odd error, when python does not think str are str
                NAMES = NAMES+","+str(sen)
            # loop through sensors to create columns
            if "SDS" in sen or "sds" in sen:
                columns = columns+",sds-pm2.5,sds-pm10,sds-TSP"
     # create the csv
    # replace the commers from the Sensors names to add tio file name
    csvnames = NAMES.replace(",", "-")
    ofile = FOLDER + LOCATION + "_" + RPI+'_' + \
        csvnames+"_" + str(date).replace('-', '') + ".csv"
...
if __name__ == "__main__":
    ...
    if V.DHTON == "ON":
        for DHTN in V.DHTNAMES:
            runsen.append(DHTN)
    print(V.RPINAME, " Starting in Mode: ", V.MODE, "Sensors:", V.RUNSEN,
          " Time: ", datetime.datetime.now(), "Location:", V.LOC[0])
    ...
    while time.time() % inter != 0:
        ...
        while True:
            ...
            if MODE == "GPS" or MODE == "TEST":  # if GPS  or a TEST add the time in mins to the file name
                f = initFile(starttime.strftime('%Y%m%d-%H%M%S'),
                             RPI, FOLDER, LOCATION, R)
                ...
                for pro, r, p in zip(Sen, R, P):  # loop through OPC
                    ...
                    if V.BLINKT == "ON":
                        ...
                        if "nan" not in str(PM).lower():  # if not nan set color
                            COLOR = 0
                            COLRVAL = {0: [0, 100, 0], 1: [0, 100, 50], 2: [
                                100, 50, 0], 3: [100, 0, 0]}
                            ...
                            clear()
                            set_pixel(
                                0, COLRVAL[COLOR][0], COLRVAL[COLOR][1], COLRVAL[COLOR][2])
                            set_pixel(
                                1, COLRVAL[COLOR][0], COLRVAL[COLOR][1], COLRVAL[COLOR][2])
                            set_pixel(
                                2, COLRVAL[COLOR][0], COLRVAL[COLOR][1], COLRVAL[COLOR][2])
                            set_pixel(
                                3, COLRVAL[COLOR][0], COLRVAL[COLOR][1], COLRVAL[COLOR][2])
                            set_pixel(
                                4, COLRVAL[COLOR][0], COLRVAL[COLOR][1], COLRVAL[COLOR][2])
                            set_pixel(
                                5, COLRVAL[COLOR][0], COLRVAL[COLOR][1], COLRVAL[COLOR][2])
                            set_pixel(
                                6, COLRVAL[COLOR][0], COLRVAL[COLOR][1], COLRVAL[COLOR][2])
                            set_pixel(
                                7, COLRVAL[COLOR][0], COLRVAL[COLOR][1], COLRVAL[COLOR][2])
                            show()
            ...
            secondsToRun = (datetime.datetime.now() -
                            starttime).total_seconds() % inter
